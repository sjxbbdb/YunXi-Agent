...
fn ensure_command_jsonl_supported(command: &CliCommand, jsonl: bool) -> Result<()> {
    ...
    match command {
        ...
        CliCommand::Sessions { .. } => bail!(
            "--jsonl is only supported for agent execution commands in v2.1.4; use --json for sessions metadata commands"
        ),
        CliCommand::Parity { .. } => bail!(
            "--jsonl is only supported for agent execution commands in v2.1.4; use --json for parity commands"
        ),
        CliCommand::Persona { .. } => bail!(
            "--jsonl is only supported for agent execution commands in v2.1.4; use --json for persona management commands"
        ),
        CliCommand::Memory { .. } => bail!(
            "--jsonl is only supported for agent execution commands in v2.1.4; use --json for memory management commands"
        ),
        CliCommand::Companion { .. } => bail!(
            "--jsonl is only supported for agent execution commands in v2.1.4; use --json for companion management commands"
        ),
        CliCommand::Controls { .. } => bail!(
            "--jsonl is only supported for agent execution commands in v2.1.4; use --json for control commands"
        ),
        CliCommand::Eval { .. } => Ok(()),
        CliCommand::Weixin { .. } => bail!(
            "--jsonl is only supported for agent execution commands in v2.1.4; use --json for weixin metadata commands"
        ),
    }
}
