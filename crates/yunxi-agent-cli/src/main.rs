...
#[command(name = "yunxi")]
#[command(version)]
#[command(about = "YunXi Agent v1.9.0 interactive terminal CLI")]
struct Cli {
    #[arg(
        long,
        value_name = "BACKEND",
...
fn ensure_command_jsonl_supported(command: &CliCommand, jsonl: bool) -> Result<()> {
    ...
    match command {
        ...
        CliCommand::Sessions { .. } => bail!(
            "--jsonl is only supported for agent execution commands in v1.9.0; use --json for sessions metadata commands"
        ),
        CliCommand::Parity { .. } => bail!(
            "--jsonl is only supported for agent execution commands in v1.9.0; use --json for parity commands"
        ),
        CliCommand::Persona { .. } => bail!(
            "--jsonl is only supported for agent execution commands in v1.9.0; use --json for persona management commands"
        ),
        CliCommand::Memory { .. } => bail!(
            "--jsonl is only supported for agent execution commands in v1.9.0; use --json for memory management commands"
        ),
    }
}
