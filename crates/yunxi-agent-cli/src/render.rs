...
pub(crate) fn print_banner(banner: &InteractiveBanner) {
    println!("YunXi Agent v1.2.1 interactive CLI");
    ...
    for event in &result.events {
        match event {
            AgentEvent::Message { content } => {
                ...
                if last_assistant_message.as_deref() != Some(content) {
                    println!("{content}");
            ...
            AgentEvent::CommandUpdated {
                ...
                if !aggregated_output.trim().is_empty()
                    && last_command_output.as_deref() != Some(aggregated_output)
                {
                    println!("{aggregated_output}");
                    last_command_output = Some(aggregated_output.clone());
                }
            }
