...
YunXi Agent v1.8.0 adds the first YunXi-owned persona and transparent memory
foundation. The goal is not to make memory invisible or automatic in a risky
way; the goal is to make persona context stable, local, inspectable, and under
user control.
...
YunXi v1.8.0 stores memory as append-only JSONL. Status changes append a new
...
- `schema_version`: v1.8.0 writes `1`.
- `id`: stable memory id.
...
4. Restored session history and mentioned file context.
...
Rule extraction runs after a completed turn and never blocks the assistant final
response.
...
YunXi v1.8.0 does not add SQLite, vector search, graph memory, relationship
