# YunXi Agent v2.1.1 至 v2.2.0 个人微信接入总纲图

制定时间：2026-07-22 10:34:40 +08:00

## 一、总目标与发布约束

本路线图以 CLI 为产品核心，为 YunXi Agent 接入**个人微信私聊入口**。到
v2.2.0 时，用户可在本机通过 YunXi CLI 扫码绑定微信 iLink Bot 身份；已配对的
个人微信用户可与同一个 YunXi 陪伴 Agent 持续对话，重启后保持会话、人格和记忆
连续，并始终沿用 YunXi 现有的工具审批机制。

本次发布线共十个独立审核版本：

| 版本 | 必须创建的 Tag | 当期核心成果 |
| --- | --- | --- |
| v2.1.1 | `v2.1.1` | 微信模块边界与 CLI 命令骨架 |
| v2.1.2 | `v2.1.2` | iLink 协议客户端与确定性 Mock 测试 |
...
yunxi weixin serve [--account default] [现有 provider/sandbox/approval 参数]
...
### v2.1.1：微信模块边界与 CLI 命令骨架

**目标**：建立单一、干净的个人微信边界，不改变已有 YunXi CLI 单次运行链路。
...
- 在 `crates/yunxi-agent-cli/src/` 新增内部 `weixin` 命令模块，并挂载到既有
  `CliCommand`；不得重写现有命令分发。
- 创建 `login`、`status`、`doctor`、`serve`、`pair`、`logout` 命令骨架。本版本只
  支持帮助和参数校验，不访问腾讯网络。
- 从当前 `run` 路径抽取私有的共享执行辅助函数，确保未来 `weixin serve` 与本地 CLI
  使用同一个 Provider 选择、Runtime 构造、错误脱敏和 JSON/JSONL 约定。
- 新增 `docs/weixin-cli.md`，说明私聊限定、未完成能力和安全存储要求；不得在文档中
  宣称微信已可用。

**参考**：Reasonix 的 adapter/gateway 分层、CowAgent 的 channel 所有权、现有 YunXi
CLI 与 stream Runtime。

**验收**：新 crate 不发起网络请求即可编译；现有 `run`、`sessions`、`persona`、
`memory`、`companion`、TUI、JSON、JSONL 回归测试不变；`yunxi weixin --help` 可用，
其余操作命令必须明确提示“v2.1.1 尚未可用”。

### v2.1.2：iLink 协议客户端与确定性 Mock 测试

**目标**：先建立可测试 Rust 协议层，再暴露登录或常驻服务。
...
- 生产地址固定为官方 iLink endpoint；仅测试构造器可注入 mock endpoint，不能暴露为
  无约束用户 CLI 参数。
- 使用维护良好的 Rust HTTP mock 测试库，覆盖请求头、授权脱敏、游标、API 错误、错误
  JSON、超时和数字 ID；不得手写 HTTP 解析器。
...
本版本不持久化状态，不调起 YunXi Runtime。

**参考**：Tencent 官方插件协议行为、Reasonix `weixin.go` 的字段容错和限时 HTTP、
Hermes 对 iLink 私聊限制的说明。

**验收**：全部 fixture 不需要腾讯凭证和公网；测试证明 token 不会进入 `Debug`、
`Display`、错误输出或 JSON snapshot；`cargo fmt`、workspace check 和 workspace tests
通过。
...
- 实现窄化 `WeixinSecretStore`，Windows 首期接入系统凭证存储。秘密键由 YunXi 安装
  标识与微信账户 ID 组成。
- 利用 YunXi 既有数据根目录解析保存非机密账户信息：账户 ID、连接状态、官方 base
  URL、创建/更新时间和凭证引用；不得硬编码 C 盘路径。
...
**验收**：Mock 覆盖等待、已扫码、重定向、确认、过期、超时、取消、凭证不可用；对日志、
诊断和账户元数据运行搜索式脱敏测试；无微信命令时原 CLI 启动完全不变。
...
- 扩展 `crates/yunxi-agent-storage`，建立版本化、原子写入的通道状态存储。不得向
  `SessionRecord` 直接塞微信字段，以保证旧 session JSON 与既有命令兼容。
- 持久化：账户元数据、`get_updates_buf` 游标、入站回执/去重键、会话绑定、加密或
  secret-backed reply context、待投递元数据和配对请求；每条记录必须有 schema version
  和更新时间。
- 账户级状态锁防止同一账户运行两个 `weixin serve`。第二进程必须退出并诊断，不能竞态
  修改游标。
- 实现 `yunxi weixin doctor` 和 `status --json`：检查存储访问、凭证引用、账户锁、私聊
  策略和最后一次脱敏错误。不得显示凭证或联系人数据。
- `logout --confirm` 只删除指定账户的微信凭证引用与微信状态；不得触及 YunXi session、
  persona memory、工作区或其他账户。
...
**目标**：安全地接收个人微信私聊，并在 YunXi Runtime 视角做到一次且仅一次处理。

**实现内容**：

- 实现前台 `yunxi weixin serve`：按 iLink `getupdates` 长轮询契约运行，尊重服务端
  timeout hint，使用带 jitter 的有界指数退避，并在 Ctrl-C 后及时停止。
- 只有当消息批次已被持久化接纳后才保存 `get_updates_buf`；在 Agent dispatch 前保存
  receipt ID，以便重启回放幂等。receipt 采用有上限的 TTL/容量清理策略。
...
- 默认 `pairing`：首次陌生私聊仅生成短时配对请求，在本地 CLI 中通过
  ...
  消息忽略，仅保留脱敏诊断。
- 增加自消息过滤、轮询健康状态、队列饱和处理；轮询任务必须在其他会话执行时继续接收
  `/stop` 或审批响应。

**参考**：Tencent `getupdates` 游标、Reasonix 轮询和去重概念、OpenAkita 的配对/IM
中断产品模式。

**验收**：本地 mock 服务覆盖批次中断重启、重复 ID、游标推进、token 过期、空轮询、
退避、Ctrl-C、群聊拒绝和未进入 Agent 的配对流程；陌生消息不得创建 YunXi session 或
任何 persona/memory 写入；既有 CLI 取消和 TUI 测试通过。
...
- 必须继续使用 YunXi 当前 approval/sandbox 配置。微信不得提升权限、保存广泛命令
  前缀，或把自然语言“好的”理解为批准。
- 覆盖过期、重复回应、陈旧回应、断连、队列满和服务关闭，避免某个 turn 因手机无回应而
  永久卡死。
...
- 加入 outbound delivery spool：消息 hash、turn ID、次数、API 返回 message ID 与最终
  失败状态。腾讯返回成功回执后才标记为 delivered。
...
**验收**：mock 覆盖 stale context、超时、4xx/5xx、重复发送、重启待投递、长 Unicode、
代码块、局部失败和状态脱敏；失败段必须在 status/diagnostic 可见且绝不向用户或 Runtime
虚报已送达；公开微信文本无推理、工具轨迹和秘密。
...
- 让稳定微信来源映射到同一个 YunXi session identity，使重启、本地 CLI 续聊和微信
  续聊中的人格/偏好/关系召回一致。
- 新增通道安全的 companion status/session reset 处理。reset 必须经本地 CLI 明确确认，
  或由已配对用户请求后再回显确认；不得静默清除人格或长期记忆。
...
只有在以下闭环全部成立时，v2.2.0 才算完成：已配对个人微信私聊进入既有 YunXi
companion Runtime；复用可持久的私密 session 和记忆；回信有真实投递依据；受控重启后
可恢复；工具仍位于明确远程审批之后；默认 CLI 与既有 TUI 保持稳定；秘密不离开本地安全
存储；群聊和未经验证的主动推送不被宣传为已完成能力。
